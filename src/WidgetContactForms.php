...
final class WidgetContactForms extends WidgetDialogBase
{
    ...
    function __construct()
    {
        ...
        $this->nameModalToggle = __("Add an Ad", 'wptheme');
    ...
    function sendMessage()
    {
        ...
        if ($hasEmail && $hasNonce &&
            ...
            if (is_email($emailTo)) {
                ...
                if (is_email($contactEmail)) {
                    $contactEmail = sprintf('<p><strong>%s</strong>:%s</p>', __("Email", 'wptheme'),
                        $contactEmail);
                }
                /** Contact Phone */
                $contactPhone = sanitize_text_field($_POST['number']);
                if (!empty($contactPhone)) {
                    $contactPhone = sprintf('<p><strong>%s</strong>:%s</p>', __("Phone Number", 'wptheme'),
                        $contactPhone);
                }
                /** Property TITLE */
                $pageUrl = isset($_SERVER['HTTP_REFERER']) ? $_SERVER['HTTP_REFERER'] : home_url();
                $pageTitle = sprintf('<p><strong>%1$s</strong>:<a href="%2$s">%2$s</a></p>',
                    __("Page Title", 'wptheme'), $pageUrl);
                /** Email SUBJECT */
                $emailSubject = sprintf('%s %s %s',
                    get_bloginfo('name'), __("message from", 'wptheme'), $contactName);
                /** Email BODY */
                ...
                if (wp_mail($emailTo, $emailSubject, $emailBody, $emailHeaders)) {
                    echo json_encode([
                        'success' => true,
                        'message' => __("Message sent successfully", 'wptheme'),
                    ]);
                    die;
                }
            }
        ...
        echo json_encode(['success' => false, 'message' => __("Message wasn't sent", 'wptheme')]);
        die;
    }
    ...
    function widget($args, $instance)
    {
        ...
        if (is_email($formEmail)) {
            ...
            $content = sprintf($markup,
                ...
                __("Send", 'wptheme')); //ReCaptcha::i()->widget()
        }
