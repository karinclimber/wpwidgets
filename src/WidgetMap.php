...
class WidgetMap extends Widget
{
    ...
    function initFields() {
        $this->addField( new WidgetField( WidgetField::RADIO, self::MAP_LOCATIONS,
            __( "Show Locations From", 'wptheme' ), [
                self::MAP_LOCATION_SITE       => __( "Site Location", 'wptheme' ),
                self::MAP_LOCATION_POST       => __( "Post Location", 'wptheme' ),
                self::MAP_LOCATION_PROPERTIES => __( "Properties", 'wptheme' ),
            ], self::MAP_LOCATION_SITE ) );
        $this->addField( new WidgetField( WidgetField::NUMBER, self::MAP_ZOOM, __( "Map Zoom", 'wptheme' ),
    ...
    function getPropertyMapData() {
        ...
        if ( $propertyMapVisible && empty( $mapCoordinates ) == false ) {
            ...
            $property['thumb']   = getGalleryFirsImage( get_the_ID() );
    ...
    function getContactMap( $mapZoom = self::MAP_ZOOM_DEFAULT ) {
        $coordinates = self::getCoordinates( get_option( ThemeSetting::SITE_ADDRESS ) );
        $lat         = $coordinates['lat'];
        $lng         = $coordinates['lng'];
