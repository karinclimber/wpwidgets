...
final class WidgetAuthors extends WidgetLayoutBase
{
    ...
    function initFields()
    {
        $this->addField(new WidgetField(WidgetField::CHECKBOX, self::SHOW_POST_AUTHOR,
            __('Show Only Post Author', 'wptheme'), [], false));
    ...
    function getCardForAuthor(WP_User $author, string $columns = "12")
    {
        ...
        //$authorPostCount = sprintf(_n('%s property', '%s properties', $authorPostCount, 'wptheme'), $authorPostCount);
        /** Mobile */
        ...
        if (is_singular() || (!$skypeUser && !$authorMobile)) {
            ...
            $widgetInstance = [
                ...
                WidgetContactForms::MODAL_TOGGLE_NAME => __("Write a letter", 'wptheme'),
            ...
            $widgetInstance = [
                ...
                Widget::CUSTOM_TITLE => __("Write a letter", 'wptheme'),
            ];
            the_widget("WidgetContactForms", $widgetInstance, [
                WPSidebar::BEFORE_WIDGET => '<section id="%s" class="widget">',
                WPSidebar::AFTER_WIDGET => '</section>',
    ...
    function widget($args, $instance)
    {
        ...
        if ($showPostAuthor) {
            $currentAuthor = WPUtils::getCurrentAuthor();
            if ($currentAuthor) {
